...
Shader "Unlit/ChromaKey"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // Get pixel width
                float2 pixelWidth = float2(1.0 / _BaseColorMap_TexelSize.z, 0);
                float2 pixelHeight = float2(0, 1.0 / _BaseColorMap_TexelSize.w);
                ...
                // Unmodified MainTex
                float4 color = tex2D(_BaseColorMap, i.uv);

                // Unfeathered mask
                float mask = maskedTex2D(_BaseColorMap, i.uv);

                // Feathering & smoothing
                float c = mask;
                float r = maskedTex2D(_BaseColorMap, i.uv + pixelWidth);
                float l = maskedTex2D(_BaseColorMap, i.uv - pixelWidth);
                float d = maskedTex2D(_BaseColorMap, i.uv + pixelHeight); 
                float u = maskedTex2D(_BaseColorMap, i.uv - pixelHeight);
                float rd = maskedTex2D(_BaseColorMap, i.uv + pixelWidth + pixelHeight) * .707;
                float dl = maskedTex2D(_BaseColorMap, i.uv - pixelWidth + pixelHeight) * .707;
                float lu = maskedTex2D(_BaseColorMap, i.uv - pixelHeight - pixelWidth) * .707;
                float ur = maskedTex2D(_BaseColorMap, i.uv + pixelWidth - pixelHeight) * .707;
